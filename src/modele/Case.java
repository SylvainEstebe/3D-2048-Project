...
public class Case implements Parametres, Serializable {
    ...
    public int scoreCase() {
        int score = 0;
        int compteur = 0;
        if (this.getVoisinDirect(DROITE) != null) {
            if (this.getVoisinDirect(DROITE).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(DROITE).getValeur());
                compteur++;
            }
        }
        if (this.getVoisinDirect(HAUT) != null) {
            if (this.getVoisinDirect(HAUT).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(HAUT).getValeur());
                compteur++;
            }
        }
        if (this.getVoisinDirect(GAUCHE) != null) {
            if (this.getVoisinDirect(GAUCHE).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(GAUCHE).getValeur());
                compteur++;
            }
        }
        if (this.getVoisinDirect(BAS) != null) {
            if (this.getVoisinDirect(BAS).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(BAS).getValeur());
                compteur++;
            }
        }
        if (this.getVoisinDirect(DESCG) != null) {
            if (this.getVoisinDirect(DESCG).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(DESCG).getValeur());
                compteur++;
            }
        }
        if (this.getVoisinDirect(MONTERG) != null) {
            if (this.getVoisinDirect(MONTERG).getValeur() != 0) {
                score += abs(this.getValeur() - this.getVoisinDirect(MONTERG).getValeur());
                compteur++;
            }
        }
        if (compteur != 0) {
